...
"""
    RadonParallelCircle(N, in_height, weight)
...
struct RadonParallelCircle{T} <: RadonGeometry
    ...
    function RadonParallelCircle(N, in_height)
        return new{eltype(in_height)}(N, in_height, in_height .* 0 .+ 1) 
    end

    function RadonParallelCircle(N, in_height, weights)
        return new{eltype(in_height)}(N, in_height, weights) 
    end
end
...
struct RadonFlexibleCircle{T} <: RadonGeometry
    ...
    function RadonFlexibleCircle(N, in_height, out_height)
        return new{eltype(in_height)}(N, in_height, out_height, in_height .* 0 .+ 1) 
    end
